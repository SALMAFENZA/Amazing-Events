...
fetch("https://mh-amazing.herokuapp.com/amazing")
  ...
  .then((data) => {
    fechas = data.date
    eventos = data.events;
    futures = eventos.filter((event) => event.date > fechas);
    crearCheckbox(futures, $checkbox);
    imprimirCards(futures, $tarjetas);
